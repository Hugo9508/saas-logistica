...
export const EmpresaEdit = () => {
  ...
  const empresaData = query?.data?.data;
        ...
        <Row gutter={16}>
          <Col xs={24} md={8}>
            <Form.Item label="Nombre" name="nombre" rules={[{ required: true }]}>
              <Select disabled={!!empresaData?.nombre} options={NOMBRE_OPTIONS} />
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
              ...
              {fields.map(({ key, name, ...restField }) => (
                <Card size="small" key={key} style={{ marginBottom: 12 }}>
                  <Row gutter={16} align="middle">
                    <Col xs={24} md={9}>
                      <Form.Item
                        {...restField} name={[name, "banco"]} label="Banco"
                    ...
                    <Col xs={24} md={9}>
                      <Form.Item
                        {...restField} name={[name, "nro_cuenta"]} label="Nro. Cuenta"
                        rules={[{ required: true, message: "Requerido" }]}
                      >
                        <Input />
                      </Form.Item>
                    ...
                    <Col xs={24} md={4}>
                      <Form.Item
                        {...restField} name={[name, "moneda"]} label="Moneda"
                        rules={[{ required: true, message: "Requerido" }]}
                      >
